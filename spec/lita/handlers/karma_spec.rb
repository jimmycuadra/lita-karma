...
describe Lita::Handlers::Karma, lita_handler: true do
  ...
  describe "#decrement" do
    ...
    it "doesn't start from zero if the term already has a positive score" do
      allow(robot).to receive(:say)
      send_test_message("foo++")
      expect(robot).to receive(:say).with("foo: 0")
      send_test_message("foo--")
    end
  end
  ...
  describe "#list" do
    ...
    it "lists the top 5 terms by default" do
      expect(robot).to receive(:say).with <<-MSG.chomp
1. one (3)
2. two (2)
3. three (1)
4. four (0)
...
MSG
  ...
  describe "#link" do
    it "says that it's linked term 2 to term 1" do
      expect(robot).to receive(:say).with("bar has been linked to foo.")
    ...
    it "says that term 2 was already linked to term 1 if it was" do
      allow(robot).to receive(:say)
      send_test_message("#{robot.name}: foo += bar")
      expect(robot).to receive(:say).with("bar is already linked to foo.")
      send_test_message("#{robot.name}: foo += bar")
    end
  ...
  describe "#unlink" do
    it "says that it's unlinked term 2 from term 1" do
      allow(robot).to receive(:say)
      send_test_message("#{robot.name}: foo += bar")
      expect(robot).to receive(:say).with("bar has been unlinked from foo.")
      send_test_message("#{robot.name}: foo -= bar")
    end

    it "says that term 2 was not linked to term 1 if it wasn't" do
      expect(robot).to receive(:say).with("bar is not linked to foo.")
      send_test_message("#{robot.name}: foo -= bar")
    end
